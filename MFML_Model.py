        ...
        **optargs : dict
        ...
        #KRR optimizer - non-linear
            ...
            defaultKwargs = {'sigma':700.0,'reg':1e-9, 
                             ...
                             'metric':'l2'}
            defaultKwargs.update(**optargs) #{**defaultKwargs, **optargs}
            if defaultKwargs['kernel_type']=='gaussian':
                K_val = kernels.gaussian_kernel(val_preds, val_preds, 
                                                sigma=defaultKwargs['sigma'])
            ...
            elif defaultKwargs['kernel_type']=='laplacian':
                K_val = kernels.laplacian_kernel(val_preds, val_preds, 
                                                 sigma=defaultKwargs['sigma'])
    
                K_eval = kernels.laplacian_kernel(val_preds, test_preds, 
                                                  sigma=defaultKwargs['sigma'])
                
            elif defaultKwargs['kernel_type']=='matern':
                K_val = kernels.matern_kernel(val_preds, val_preds, 
                                              sigma=defaultKwargs['sigma'], 
                                              order=defaultKwargs['order'], metric = defaultKwargs['metric'])
    
                K_eval = kernels.matern_kernel(val_preds, test_preds, 
                                               sigma=defaultKwargs['sigma'],
                                               order=defaultKwargs['order'], metric = defaultKwargs['metric'])
            ####default to linear kernel
            else:
                K_val = kernels.linear_kernel(val_preds, val_preds)
                K_eval = kernels.linear_kernel(val_preds, test_preds)
        ...
        ####### KRR optimizer with feature kernel - composite KRR  
            ...
            ####default to linear kernel
                ...
                K_eval = kernels.linear_kernel(val_preds, test_preds)
            ###########
            #The input feature (X) kernel is the same as the one used in the regular MFML process
            #so we call that to compute the input features kernels
            K_x_val = self.kernel_generators(X1 = X_val,
                                             X2 = X_val)
            K_x_eval = self.kernel_generators(X1 = X_val,
                                             X2 = X_test)
            ### combine stuff
            K_val_composite = np.multiply(K_val,K_x_val) #elemenet-wise multiplication
            K_eval_composite = np.multiply(K_eval, K_x_eval)
            ###solve system
            K_val_composite[np.diag_indices_from(K_val_composite)] += defaultKwargs['reg']
            solved_coeffs = cho_solve(K_val_composite, y_val)
            final_preds = np.dot(solved_coeffs,K_eval_composite)
            self.coeffs = np.copy(solved_coeffs)
